...
import z from "zod";
import type { Repository } from "../page";
...
export function CreateProjectFormPrimitive({
	...
	const syncSourceFromTarget = (targetId: string) => {
	...
	const targetComponent = ({ ...props }: React.ComponentProps<typeof FieldGroup>) => (
		<FieldGroup {...props}>
			<form.Field
			...
			>
				{(field) => {
					...
					const selectedRepository = _repositories.find((r) => r.id === field.state.value) ?? null;
								...
								<ComboboxTrigger
									render={
										<Button
											...
											variant="outline"
											className={"w-full justify-start font-normal data-placeholder:text-zinc-500"}
										>
			...
			<form.Field name="framework">
				{(field) => {
					const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid;
					const selectedFramework = frameworks.find((f) => f.value === field.state.value) ?? null;
	...
	const sourceComponent = ({ ...props }: React.ComponentProps<typeof FieldGroup>) => (
		<FieldGroup {...props}>
					...
					<>
						<form.Field
							...
							listeners={{
								onChange: ({ value }) => {
									if (value) {
										form.setFieldValue("sourceRepository", form.getFieldValue("targetRepository"));
									}
								},
							}}
						>
							{(field) => (
								<Field orientation={"horizontal"}>
									...
									<FieldContent>
										...
										<FieldDescription>
											When enabled, the source repository will be the same as the target repository.
										</FieldDescription>
									</FieldContent>
								</Field>
							)}
						...
						<form.Field name="sourceRepository">
							{(field) => {
								...
								const selectedRepository =
									_repositories.find((r) => r.id === field.state.value) ?? null;
	...
	const completeComponent = ({ ...props }: React.ComponentProps<typeof FieldGroup>) => (
		<FieldGroup {...props}>
			<Button type="submit">Complete</Button>
		</FieldGroup>
	);

	const formComponent = ({ onSubmit, ...props }: React.ComponentProps<"form">) => (
		<form
			onSubmit={(e) => {
				e.preventDefault();
				form.handleSubmit();
